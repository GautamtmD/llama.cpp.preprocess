...
def _inject_audio(base, sid, name, *, system_prompt=TRANSCRIPTION_SYSTEM_PROMPT, extra_text=None):
...
def test_inject_mixed_text_image_audio(base, make_session):
    ...
    sid = make_session()
    img = io.BytesIO()
    Image.new("RGB", (224, 224), (0, 0, 255)).save(img, "PNG")
...
def test_asr_transcribes_fixture(base, make_session, name):
    ...
    print(
        f"  [asr:{name}] answer  ={answer[:12]}{'...' if len(answer) > 12 else ''} "
        f"(clean_match={clean})"
    )
